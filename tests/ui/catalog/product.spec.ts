...
test.describe('Product Page', { tag: ['@catalog'] }, () => {
    ...
    test('Product details are displayed correctly', { tag: '@smoke' }, async ({ page }) => {
        ...
        const product = productPage.product_card_title.filter({
            hasText: 'Vortex Mechanical Keyboard',
        });
        ...
        await expect(productPage.product_description).toHaveText(
            'Hot-swappable RGB mechanical keyboard with tactile switches.',
        );
    ...
    test('Out of stock Product', async () => {
        ...
        const product = productPage.product_card_title.filter({
            hasText: 'SoundWave Bluetooth Speaker',
        });
...
test.describe('Product Detail Page - Error Handling', { tag: ['@catalog'] }, () => {
    test('Should display not-found state and hide add-to-cart button when product API returns 500', async ({
        page,
    }) => {
        ...
        await page.route(
            (url) => url.pathname === `/api/products/${id}`,
            async (route) => {
                await route.fulfill({
                    status: 500,
