...
test.describe('Home Page', { tag: ['@catalog'] }, () => {
    ...
    test('Sort by price (low to high)', { tag: '@sanity' }, async () => {
        const priceBeforeSort = await homePage.product_card_price.first().innerText();
        await homePage.product_sort.selectOption('Price: Low to High');
        await expect(homePage.product_card_price.first()).not.toHaveText(priceBeforeSort);
    });
...
test.describe('Home Page Network Failures', { tag: ['@catalog'] }, () => {
    ...
    test('Should keep products grid working even if categories API fails with 500', async ({
        page,
    }) => {
        await page.route(
            (url) => url.pathname === '/api/products/categories',
            async (route) => {
                await route.fulfill({
                    status: 500,
                    contentType: 'application/json',
